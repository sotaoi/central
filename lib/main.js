...
const main = async () => {
  ...
  const startServer = async (app) => {
    ...
    const redirectFromPort = Config.env('REDIRECT_FROM_PORT');
    if (!!redirectFromPort) {
      ...
      expressrdr.listen(redirectFromPort);
      console.info(`[${getTimestamp()}] Server redirecting from port ${redirectFromPort}`);
    }
  };
